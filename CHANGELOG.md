...
- Agent Mental receipt no longer wraps with `────────`. Cursor markdown joined those lines into the same paragraph as the title.
- Agent Mental receipt items are stacked (one line each, flush left, two trailing spaces) so Cursor does not join `📓 Journal` and `🔍 Read` onto one line.
